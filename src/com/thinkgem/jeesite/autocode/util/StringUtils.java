...
public class StringUtils {
    ...
    public static String getJavaName(String fromName){
    	String javaName = "";
		String[] segments = fromName.split("_");
		for (int i = 0; i < segments.length; i++) {
			String segment = segments[i];
			if(i==0&&segment.length()==1){
				continue;
			}
			if(StringUtils.isBlank(javaName)){
				javaName += StringUtils.uncapitalize(segment);
			}else{
				javaName += StringUtils.capitalize(segment);
			}
		}
		return javaName;
    }
